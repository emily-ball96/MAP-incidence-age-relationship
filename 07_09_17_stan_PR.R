...
init <- function(){list("kappa" = 1, "sigma1" = 1, "sigma2" = rep(1,3), p = 0.05, beta1 = rep(0, 3), beta2 = rep(0,3), beta3 = rep(0,3), "eta" = log(data3$PYO))}

fit <- stan(
  file = "07_09_17_PR.stan",                              # Stan program
  data = incidence_data,                                  # named list of data
  chains = 1,                                             # number of Markov chains
  warmup = 20,                                            # number of warmup iterations per chain
  iter = 40,                                              # total number of iterations per chain
  cores = 4,                                              # number of cores
  refresh = 500,                                          # show progress of chains every 'refresh' iterations
  control = list(adapt_delta = 0.8, max_treedepth = 12),  # to avoid divergent transitions
  init = init
)
...
print(fit, pars = c("lp__", "nu", "eps[1]"))    # want large n_eff and rhat ~ 1
