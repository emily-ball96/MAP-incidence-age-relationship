...
points(x=asia_data$AGE, y = asia_data$d, pch = 20, col = "lightgreen")
...
alpha <- 0.1
beta <- 0.1
c1 <- qgamma(0.05, asia_data$d + alpha, asia_data$PYO + beta)
c2 <- qgamma(0.95, asia_data$d + alpha, asia_data$PYO + beta)
##arrows(asia_data$AGE, asia_data$d - c1, asia_data$AGE, asia_data$d + c2, length = 0.01, angle = 90, code = 3)