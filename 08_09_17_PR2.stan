...
transformed parameters{
  ...
  log_p = log(p);                       // for efficiency, transform parameters now, not in model block
  log_1_p = log(1-p);                   // 
}
...
generated quantities{
  ...
  alpha_new_af = log(1000) + beta1[1] + beta2[1] * a_new + beta3[1] * asq_new;        // to predict inc at ages 0-85
  alpha_new_am = log(1000) + beta1[2] + beta2[2] * a_new + beta3[2] * asq_new;        // to predict inc at ages 0-85
  alpha_new_as = log(1000) + beta1[3] + beta2[3] * a_new + beta3[3] * asq_new;        // to predict inc at ages 0-85
}
