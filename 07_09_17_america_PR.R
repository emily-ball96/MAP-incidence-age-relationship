...
points(x=america_data$AGE, y = america_data$INC, pch = 20, col = "lightgreen")
...
alpha <- 0.1
beta <- 0.1
c1 <- qgamma(0.05, america_data$INC + alpha, america_data$PYO + beta)
c2 <- qgamma(0.95, america_data$INC + alpha, america_data$PYO + beta)
##arrows(america_data$AGE, america_data$INC - c1, america_data$AGE, america_data$INC + c2, length = 0.01, angle = 90, code = 3)