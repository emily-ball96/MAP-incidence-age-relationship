...
as_data <- data3[data3$REGION == "CSE Asia", ]
q_as <- y_pred_q[,1:length(as_data$AGE)]                            # quantiles
AGE <- as_data$AGE
all_q <- cbind(AGE, as_data$d, q_as[1,], q_as[2,], q_as[3,])
AGE <- sort(AGE)
